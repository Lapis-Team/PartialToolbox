...
theorem div_existence{x y : Option Nat}: isdef (x / y) -> y ≠ 0 :=
 match x,y with
   | .some x, .some y => by
      intro h
      change isdef (if y==0 then none else some (x/y)) at h
      split at h
      . apply False.elim h
      . change some y ≠ some 0
        grind
   | .none, _ => False.elim
   | .some _, .none => False.elim

theorem ex1 {x y : Option Nat} : isdef x -> isdef y -> y ≠ .some 0 -> (x / y) * y <= x := by
  apply isdef_elim ; intro x
  apply isdef_elim ; intro y
  intro h
  have h : y≠0 := fun a => h (congrArg _ a)
  change (if y == 0 then .none else .some (x / y)) * some y ≤ some x
