...
- Existence conditions are defined by means of the `Existence` type-class.
  An instance in the form `Existence t P` asserts that if the term `t` is defined
  then the predicate `P` must hold.

- Given a relation `R`, the directed relations built starting from `R` ensure that `R`
    holds if one or both the terms are defined. In particular we define the 
    left-to-right (`ltor`), right-to-left (`rtol`) and bidirectional (`bidir`) variants. 
    The transitivities that hold for combinations of such directions are also shown. 
...
- Example usage
  To register partiality over a type we need to instantiate the `Partial` type-class.
    ...
    `instance : Partial (Option α) where isdef x := x.isSome`
  ...
  To declare existence conditions for a term `t` we need to register the proper `Existence` instance.
    For example, we may declare that if the expression `x / y` is defined then the denominator must be different from 0 as follows
    `Existence (x / y) (y ≠ 0)`
-/
